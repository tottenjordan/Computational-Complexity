# -*- coding: utf-8 -*-
"""
Created on Wed May 23 21:14:07 2018
...
#####################################################################################################
# write distance to CSV
#import csv

#with open('thirty_dist.csv', 'wb') as f:
 #   w = csv.DictWriter(f, dist.keys())
  #  w.writeheader()
   # w.writerows(dist)
 
pd.DataFrame(dist)
distance = pd.DataFrame(dist.items())    
distance.to_csv('distance1.csv')

distance.columns = ["location1", "distance"]

distance['location2'], distance['location3'] = zip(*distance['location1'].map(lambda x: x.split(',')))
